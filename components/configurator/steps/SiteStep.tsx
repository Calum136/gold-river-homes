...
export type SiteSection = "foundation" | "water" | "sewer";
...
export default function SiteStep({ state, onChange, only }: SiteStepProps) {
  ...
  const sewer = (
    <>
      ...
      {/* Auto-estimated costs — included with the final site decision */}
      <div className="border border-border bg-bg-elevated mt-4">
      ...
      <p className="text-text-muted/40 text-xs leading-snug mt-3">
  ...
  if (only) {
    const sections: Record<SiteSection, React.ReactNode> = { foundation, water, sewer };
    return (
      <div>
        {preSelectedFromLot && only !== "foundation" && (
          <p className="text-text-muted/60 text-xs mb-3">Your lot's available services have been pre-selected.</p>
        )}
        <div className="border border-border bg-bg-secondary p-4">{sections[only]}</div>
      </div>
    );
  }
